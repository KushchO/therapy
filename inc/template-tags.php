...
if ( ! function_exists( 'maximus_posted_on' ) ) :
	...
	function maximus_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'maximus' ),
...
if ( ! function_exists( 'maximus_posted_by' ) ) :
	...
	function maximus_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'maximus' ),
...
if ( ! function_exists( 'maximus_entry_footer' ) ) :
	...
	function maximus_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'maximus' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'maximus' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'maximus' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'maximus' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
